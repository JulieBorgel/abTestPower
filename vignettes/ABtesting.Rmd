...
```{r}
bayesABtests |>
  unnest_longer(c(credibleIntervalQ5, credibleIntervalQ90), indices_include = TRUE, indices_to = "index", names_repair =  "universal") |>
  mutate(y = (credibleIntervalQ90 - credibleIntervalQ5)/2) |>
ggplot(aes(x = index...5, y = y, ymin = credibleIntervalQ5, ymax = credibleIntervalQ90)) +
  geom_hline(yintercept = c(0, .5), color = "#39778A") +
  geom_pointrange(fatten = 1/2) +
  facet_grid(topScorerSampleSize~lowScorerSampleSize) +
  labs(x = "simulation index",
       y = "Credible Interval on (A - B) / B for interval length(s) (0.9)")


bayesABtests |>
  unnest_longer(c(credibleIntervalQ5, credibleIntervalQ90), indices_include = TRUE, indices_to = "index", names_repair =  "universal") |>
  group_by(topScorerSampleSize, lowScorerSampleSize, probability) |>
  mutate(check = ifelse(probability > 0, 1, 0)) |>
